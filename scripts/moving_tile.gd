...
@export var neighbor_sets : TileArray2D
...
func activate():
	...
	# cycles the set of neighbors to the next list
	# which should correspond to the ending position's
	# neigbors
	neighbor_sets.append(neighbors)
	neighbors = neighbor_sets.pop()
